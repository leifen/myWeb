...
<!-- dcHead 结束 -->
    ...
    <div class="wrap mb">
        <div id="pageLeft">
            ...
            <div class="treeBox">
                <h3><?php echo L('article_tree');?></h3>
                <ul>
                    <?php if(is_array($article_category)): $i = 0; $__LIST__ = $article_category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i; if($cate['cur']): ?><li class="cur"><?php else: ?><li><?php endif; ?>
                        ...
                        <?php if($cate['child']): ?><ul>
                                <?php if(is_array($cate['child'])): $i = 0; $__LIST__ = $cate['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i; if($child['cur']): ?><li class="cur"><?php else: ?><li><?php endif; ?>
                                    -<a href="<?php echo ($child["url"]); ?>"><?php echo ($child["cat_name"]); ?></a>
                                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
        ...
        <div id="pageIn">
            ...
            <div class="urHere"><?php echo L('ur_here');?>：
                ...
                <a href="<?php echo ($system_module["article_category"]); ?>"><?php echo L('article_category');?></a>
                <?php if($ur_here): ?><b>></b><?php echo ($ur_here); endif; ?>
            </div>

            <div id="article">
                <h1><?php echo ($article["title"]); ?></h1>
                <div class="info"><?php echo L('add_time');?>：<?php echo (date("Y-m-d",$article["add_time"])); ?> <?php echo L('click');?>：<?php echo ($article["click"]); ?>
                    <?php if(!empty($article['defined'])): if(is_array($article['defined'])): $i = 0; $__LIST__ = $article['defined'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$defined): $mod = ($i % 2 );++$i;?><li><?php echo ($defined["arr"]); ?>：<?php echo ($defined["value"]); ?></li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                </div>
                <div class="content"> <?php echo ($article["content"]); ?> </div>

                <div class="lift">
                    <?php if($pre_next['pre']['id']): ?><span><?php echo L('article_previous');?>：<a href="<?php echo ($pre_next["pre"]["url"]); ?>"><?php echo ($pre_next["pre"]["title"]); ?></a></span><?php endif; ?>
                    <?php if($pre_next['next']['id']): ?><span><?php echo L('article_next');?>：<a href="<?php echo ($pre_next["next"]["url"]); ?>"><?php echo ($pre_next["next"]["title"]); ?></a></span><?php endif; ?>
                </div>
            </div>
        </div>
        <div class="clear"></div>
